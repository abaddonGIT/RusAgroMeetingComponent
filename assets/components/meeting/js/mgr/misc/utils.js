Meeting.utils.renderBoolean = function (value) {
    return value
        ? String.format('<span class="green">{0}</span>', _('yes'))
        : String.format('<span class="red">{0}</span>', _('no'));
};

Meeting.utils.getMenu = function (actions, grid, selected) {
    var menu = [];
...
Meeting.utils.renderActions = function (value, props, row) {
    var res = [];
    ...
    return String.format(
        '<ul class="meeting-row-actions">{0}</ul>',
        res.join('')
    );
};
